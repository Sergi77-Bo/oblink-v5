...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Fallback: If intersection observer doesn't trigger in 1s, force show
  setTimeout(() => {
    document.querySelectorAll('.counter').forEach(counter => {
      if (counter.textContent === '0' || counter.textContent === '0,0') {
        let targetAttr = counter.getAttribute('data-target');
        if (targetAttr) {
          counter.textContent = targetAttr.replace('.', ',');
          // Check if it needs formatting (thousands separator)
          const val = parseFloat(targetAttr.replace(',', '.'));
          if (!isNaN(val) && val > 999) {
            counter.textContent = val.toLocaleString('fr-FR');
          }
        }
      }
    });
  ...
  function animateCounter(element, start, end, duration, isDecimal) {
    ...
    const step = (timestamp) => {
      ...
      if (isDecimal) {
        // Replace dot with comma for FR and keep 1 decimal
        element.textContent = current.toFixed(1).replace('.', ',');
      } else {
        // Use French grouping for integers
  ...
  ctaButtons.forEach(btn => {
    btn.addEventListener('click', function (e) {
      ...
      // TODO: Send to analytics (gtag, Plausible, etc.)
  ...
  if (conversationList) {
    ...
    conversationList.addEventListener('click', function (e) {
      ...
      // Load Messages AJAX
      chatMessages.innerHTML = '<div class="flex h-full items-center justify-center text-gray-400 gap-2"><i class="fas fa-spinner fa-spin text-2xl"></i> Chargement...</div>';

      retryFetch(oblink_vars.ajax_url + '?action=oblink_load_chat&contact_id=' + contactId + '&nonce=' + oblink_vars.nonce)
        ...
        .then(data => {
          if (data.success) {
            ...
            chatMessages.innerHTML = '<div class="flex h-full items-center justify-center text-red-500 flex-col"><i class="fas fa-exclamation-triangle mb-2"></i> Erreur chargement</div>';
          }
        })
        .catch(err => {
          console.error('Chat load error:', err);
          chatMessages.innerHTML = '<div class="flex h-full items-center justify-center text-red-500 flex-col"><i class="fas fa-wifi mb-2"></i> Problème de connexion. <button onclick="this.closest(\'.conversation-item\').click()" class="mt-2 text-sm underline">Réessayer</button></div>';
        });
    });
...
`;
  ...
  if (window.performance) {
    window.addEventListener('load', () => {
      setTimeout(() => {
        ...
        console.log('📊 OBLINK Performance:', {
          'Page Load': (pageLoadTime / 1000).toFixed(2) + 's',
          'Server Response': (connectTime / 1000).toFixed(2) + 's',
          'DOM Ready': ((perfData.domContentLoadedEventEnd - perfData.navigationStart) / 1000).toFixed(2) + 's'
        });
  ...
  // ===== MODAL UX IMPROVEMENTS (Audit 2025) =====
  class Modal {
    constructor(modalId) {
    ...
    init() {
      ...
      // Global Escape listener (only active when modal is open)
      document.addEventListener('keydown', (e) => {
        if (this.isOpen && e.key === 'Escape') this.close();
        if (this.isOpen && e.key === 'Tab') this.trapFocus(e);
      });
    }

    open() {
      this.modal.classList.remove('hidden');
      this.modal.classList.add('flex'); // Assume flex centering
      this.isOpen = true;
      document.body.classList.add('modal-open'); // Scroll lock (CSS)
