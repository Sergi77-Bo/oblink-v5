...
// Ensure data is array, not null
$crm_magasins = is_array($crm_magasins) ? $crm_magasins : [];
$crm_opticiens = is_array($crm_opticiens) ? $crm_opticiens : [];
$tasks = is_array($tasks) ? $tasks : [];
$sprint = is_array($sprint) ? $sprint : [];

// Calculate KPIs for Overview
$total_prospects = count(array_filter($crm_magasins, fn($i) => isset($i['status']) && $i['status'] === 'prospect')) +
    count(array_filter($crm_opticiens, fn($i) => isset($i['status']) && $i['status'] === 'prospect'));
$total_active = count(array_filter($crm_magasins, fn($i) => isset($i['status']) && $i['status'] === 'active')) +
    count(array_filter($crm_opticiens, fn($i) => isset($i['status']) && $i['status'] === 'active'));
